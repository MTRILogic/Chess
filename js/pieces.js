...
export class King extends Piece {
  ...
  getValidMoves(board, position) {
    const moves = this.generateMoves(board, position, DIRECTIONS.KING, 1);
    ...
    if (!this.hasMoved) {
      ...
      // Kingside castling
      if (this.canCastle(board, position, true)) {
        moves.push({
          from: { row, col },
          to: { row, col: col + 2 },
      ...
      // Queenside castling
      if (this.canCastle(board, position, false)) {
        moves.push({
          from: { row, col },
          to: { row, col: col - 2 },
  ...
  canCastle(board, position, kingSide) {
    ...
    const endCol = kingSide ? rookCol - 1 : rookCol + 1;
    for (let c = col + direction; c !== endCol + direction; c += direction) {
      if (board[row][c]) {
        return false;
      }
    }
    ...
    // This will be handled by the board class when filtering valid moves
